...
- `reject` → block the packet, log it, and terminate the session

### Protocol

- Specifies which protocol the rule applies to (TCP, UDP, ICMP, etc.)
...
- Snort is widely used in SOC environments for network-based threat detection.
- It helps analysts identify malicious traffic patterns and investigate suspicious activity.
...
- `sudo snort -c /etc/snort/snort.conf -A cmg` → detailed packet and rule output

---

## Personal notes

- Snort provides strong visibility into network traffic and detection rules.
- Understanding how Snort rules work makes it easier to detect malicious traffic patterns.# Snort
...
- Snort provides strong visibility into network traffic and detection rules.
- Understanding how Snort rules work makes it easier to detect malicious traffic patterns.
